...
export default function ExportarPanel({ 
  ...
  categoriasDinamicas = {}, 
  zonasOpciones = [],
  abrirNuevoProcesoConSeleccionados // <-- PROPIEDAD RESTAURADA AL FLUJO ORIGINAL
}) {
  ...
  const prevenirCSVInjection = (texto) => {
    ...
    if (/^[=+\-@]/.test(textoString)) return `'${textoString}`;
  ...
  const exportarCSV = () => {
    ...
    dataAExportar.forEach(p => { 
      const nombreC = prevenirCSVInjection(p.nombre_fantasia.replace(/"/g, '').replace(/,/g, ' '));
      const contactoC = prevenirCSVInjection(p.nombre_contacto.replace(/"/g, '').replace(/,/g, ' '));
      const emailC = prevenirCSVInjection(p.email_principal.replace(/"/g, '').replace(/,/g, ' '));
      csvC += `,${nombreC},${contactoC},${emailC},,\n`; 
    });
  ...
  // ADAPTACIÓN: Se envían los nombres al padre (orquestador) para que este gestione el modal global.
