...
export default function GeneradorFT() {
  ...
  // --- CONTROLES DE SEGURIDAD: LÍMITE DE BÚFER DE MEMORIA (5MB) ---
  const manejarCargaArchivos = (e) => {
    const nuevosArchivos = Array.from(e.target.files);
    
    // Filtro de seguridad (Anti DoS Local)
    const archivosSeguros = nuevosArchivos.filter(file => {
      if (file.size > 5 * 1024 * 1024) {
        alert(`⚠️ Seguridad: El archivo ${file.name} excede el límite de 5MB y fue bloqueado.`);
        return false;
      }
      return true;
    });

    setArchivosContexto([...archivosContexto, ...archivosSeguros]);
    e.target.value = null; 
  };
  ...
  const manejarCargaImagen = (e) => {
    ...
    if (file) {
      // Filtro de seguridad (Evitar desbordamiento de lienzo)
      if (file.size > 5 * 1024 * 1024) {
        return alert("⚠️ Seguridad: La imagen excede el límite de 5MB.");
      }
  ...
  // --- EXPORTACIÓN A PDF EXACTO CON BLINDAJE SRI ---
  const exportarPDF = async () => {
    if (!window.html2pdf) {
      await new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = 'https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js';
        // Ciberseguridad: Subresource Integrity (SRI) previene Supply Chain Attacks
